...
# Test 6: GET /users/route/101 where ID>100 (should use newUserService)
echo "测试 6: GET /api/users/route/101 (ID>100 -> new service)"
result6=$(curl -s -w "\n%{http_code}" http://localhost:8084/api/users/route/101)
code6=$(echo "$result6" | tail -n1)
body6=$(echo "$result6" | head -n-1)
echo "  HTTP $code6: $body6"
echo ""

# Test 7: GET /users/check/1 with isVip=true and version=v2 (should use newUserService)
echo "测试 7: GET /api/users/check/1?isVip=true (version=v2 -> new service)"
result7=$(curl -s -w "\n%{http_code}" -H "version: v2" "http://localhost:8084/api/users/check/1?isVip=true")
...
# Test 9: POST /users without source param (should use oldUserService)
...
# Test 10: POST /users with source=third-party (should use newUserService)
echo "测试 10: POST /api/users?source=third-party (-> new service)"
result10=$(curl -s -w "\n%{http_code}" -X POST -H "Content-Type: application/json" -d '{"name":"Test User 10","email":"test10@example.com"}' "http://localhost:8084/api/users?source=third-party")
...
# Test 13: GET /users/description with new=true (should use newUserService)
echo "测试 13: GET /api/users/description?new=true (-> new service)"
result13=$(curl -s -w "\n%{http_code}" "http://localhost:8084/api/users/description?new=true")
